...
#define TEST_INFO_FILENAME "test_case.json"
#define TEST_INFO_LOOKUP_PATTERN "^persist.vendor\\.crashlogd\\.TC\\."
...
unsigned long long get_uptime(int refresh, int *error)
{
    ...
    *error = 0;
    ...
    if (result < 0 && clock_gettime(CLOCK_BOOTTIME, &ts) != 0) {
        *error = errno;
        LOGE("%s - Unable to retrieve uptime: %s!\n", __FUNCTION__, strerror(errno));
        return -1;
    }
...
void do_last_kmsg_copy(char *dir) {
    ...
    if ( file_exists(LAST_KMSG) ) {
        ...
        snprintf(destion, sizeof(destion), "%s/%s", dir, CONSOLE_RAMOOPS_FILE);
        do_copy_tail(CONSOLE_RAMOOPS, destion, MAXFILESIZE);
    } else if ( file_exists(CONSOLE_RAMOOPS_NUM(0)) ) {
        snprintf(destion, sizeof(destion), "%s/%s", dir, CONSOLE_RAMOOPS_FILE);
        do_copy_tail(CONSOLE_RAMOOPS_NUM(0), destion, MAXFILESIZE);
    }
...
void do_last_fw_msg_copy(char *dir) {
    ...
    if (file_exists(CURRENT_PROC_OFFLINE_SCU_LOG_NAME)) {
        snprintf(destion, sizeof(destion), "%s/%s.txt", dir, OFFLINE_SCU_LOG_NAME);
        do_copy_eof(CURRENT_PROC_OFFLINE_SCU_LOG_NAME, destion);
    }

    if (file_exists(LM_DUMP_FILE)) {
        snprintf(destion, sizeof(destion), "%s/lkm.bin", dir);
        do_copy_eof(LM_DUMP_FILE, destion);
    }

#ifndef CONFIG_SSRAM_CRASHLOG
    if (file_exists(ACPI_BERT_TABLE_FILE))
        do_bertdump_copy(dir);
#else
    if (check_fw_crashlog()) {
        do_fw_crashlog_copy(dir);
    }
#endif
}
...
int is_event_type_fake(const char *type) {
    char value[PROPERTY_VALUE_MAX];
    char *filter = value;
    int index = 0;
    int loop = 1;

    if ((type == NULL) || strstr(type, FAKE_EVENT_SUFFIX) ||
        (!property_get(PROP_REPORT_GENERIC_FAKE, value, NULL) || !strncmp(value, "", PROPERTY_VALUE_MAX)))
        return 0;
...
static void attach_test_info(char *dir) {
    FILE *fp;
    char path[PATHMAX];
    char *props_json = retrieve_props_json(TEST_INFO_LOOKUP_PATTERN);

    if (props_json == NULL) {
        return;
    }
    // if no properties are available, skip creation of file
    else if (strlen(props_json) <= 2) {
        free(props_json);
        return;
    }

    snprintf(path, sizeof(path), "%s/%s", dir, TEST_INFO_FILENAME);
...
static int priv_raise_event(char *key, char *event, char *type, char *subtype, char *log,
    ...
    conditional_ingredients_refresh();
    ...
    fake_event = is_event_type_fake(type);
    if (fake_event != 0) {
        char *temp = malloc(strlen(type) + sizeof(FAKE_EVENT_SUFFIX));
        if (temp != NULL) {
            strcpy(temp, type);
            strcat(temp, FAKE_EVENT_SUFFIX);
            type = temp;
        } else {
            LOGE("%s failed: Could not allocate sufficient memory\n"
                "Fake event: %s - %s\n", __FUNCTION__, type, key);
            fake_event = 0;
        }
    }
...
int raise_event_nouptime(char *key, char *event, char *type, char *subtype, char *log) {
    return priv_raise_event(key, event, type, subtype, log, NO_UPTIME, 1, NULL , NULL, NULL);
}

int raise_event_wdt(char *key, char *event, char *type, char *subtype, char *log) {
    return priv_raise_event(key, event, type, NULL, log, UPTIME, 1, subtype, NULL, NULL);
}

int raise_event(char *key, char *event, char *type, char *subtype, char *log) {
    return priv_raise_event(key, event, type, subtype, log, UPTIME, 1, NULL, NULL, NULL);
}

int raise_event_bootuptime(char *key, char *event, char *type, char *subtype, char *log) {
    return priv_raise_event(key, event, type, subtype, log, UPTIME_BOOT, 1, NULL , NULL, NULL);
}

int raise_event_dataready(char *key, char *event, char *type, char *subtype, char *log,
        int data_ready) {
    return priv_raise_event(key, event, type, subtype, log, UPTIME, data_ready, NULL , NULL, NULL);
}

int raise_event_dataready012(char *key, char *event, char *type, char *subtype, char *log,
        int data_ready, char* data0, char* data1, char* data2) {
    return priv_raise_event(key, event, type, subtype, log, UPTIME,
            data_ready, data0 , data1, data2);
}
...
void check_running_power_service() {
    ...
    property_get("persist.vendor.service.power.enable", powerenable, "");
...
int process_info_and_error(char *filename, char *name) {
    ...
    key = generate_event_id(name_event, tmp);
    ...
    if (!dir) {
        ...
        raise_event(key, name_event, tmp, NULL, NULL);
    ...
    raise_event(key, name_event, type, NULL, dir);
    LOGE("%-8s%-22s%-20s%s %s\n", name_event, key, get_current_time_long(0), type, dir);
    free(key);
    free(dir);
    return 0;
}
...
void notify_crashreport_thread() {
    ...
    struct timespec ts;
    static time_t last_notify = -MIN_WAIT;
    static int thread_count = 0;
    static int last_chance = 0;
    ...
    if (thread_count >= MAX_THREAD) {
        LOGE("Too many threads, can't notify crashreport : %d\n", thread_count);
        pthread_mutex_unlock(&lock);
        return;
    }
    clock_gettime(CLOCK_MONOTONIC, &ts);
    if ((ts.tv_sec < last_notify + MIN_WAIT) && (last_chance != 0)) {
        LOGI("Already notified, can't notify crashreport.\n");
        pthread_mutex_unlock(&lock);
        return;
    }

    // if we increment thread_count, it means system command will be executed sooner or later
    thread_count++;

    if (ts.tv_sec >= last_notify + MIN_WAIT) {
        ...
        pthread_mutex_lock(&lock);
        last_chance = 0;
        LOGI("Performing last_chance notify crashreport.\n");
    }

    pthread_mutex_unlock(&lock);

    int status = run_command("am broadcast -n com.intel.crashreport/.specific.NotificationReceiver -a com.intel.crashreport.intent.CRASH_NOTIFY -c android.intent.category.ALTERNATIVE", 30);
...
void notify_crashreport() {
    ...
    if (!is_crashreport_available()) {
        LOGW("%s: Crashreport notification (CRASH_NOTIFY) skipped!\n", __FUNCTION__);
        return;
    }
...
int notify_partition_error(enum partition_error type) {
    ...
    if (!is_crashreport_available()) {
        LOGW("%s: Crashreport notification (REPORT_ERROR) skipped!\n", __FUNCTION__);
        return 0;
    }

    switch(type) {
        case ERROR_LOGS_MISSING :
             status = run_command("am broadcast -a intel.intent.action.phonedoctor.REPORT_ERROR --es \"intel.intent.extra.phonedoctor.TYPE\" \"PARTITION\" --es \"intel.intent.extra.phonedoctor.DATA0\" \"LOGS MISSING\"", 30);
             break;
        case ERROR_LOGS_RO :
             status = run_command("am broadcast -a intel.intent.action.phonedoctor.REPORT_ERROR --es \"intel.intent.extra.phonedoctor.TYPE\" \"PARTITION\" --es \"intel.intent.extra.phonedoctor.DATA0\" \"LOGS READ ONLY\"", 30);
             break;
        case ERROR_PARTITIONS_MISSING :
             status = run_command("am broadcast -a intel.intent.action.phonedoctor.REPORT_ERROR --es \"intel.intent.extra.phonedoctor.TYPE\" \"PARTITION\" --es \"intel.intent.extra.phonedoctor.DATA0\" \"PARTITION MISSING\"", 30);
             break;
        default :
             break;
    }
...
void monitor_crashenv()
{
    ...
    snprintf(cmd, sizeof(cmd), MONITOR_CRASHENV_FILE " %s", parameters);
    LOGD("%s: execute %s ", __FUNCTION__, cmd);
    int status = run_command(cmd, 45);
    if (status != 0)
...
int raise_infoerror(char *type, char *subtype) {
    ...
    key = generate_event_id(type, subtype);
    if (raise_event(key, NULL, type, subtype, LOGINFO_DIR) == 0) {
        free(key);
        return -errno;
    }
...
void check_crashlog_died()
{
    ...
    property_get("vendor.crashlogd.token", token, "");
    if (!strcmp(token, CRASHLOG_ERROR_RESET)) {
         raise_infoerror(ERROREVENT, CRASHLOG_ERROR_RESET);
         property_set("vendor.crashlogd.token", "1");
    } else if ((strlen(token) < 4)) {
         strcat(token, "1");
         property_set("vendor.crashlogd.token", token);
